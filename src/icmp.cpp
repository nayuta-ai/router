...
void icmp_input(uint32_t source, uint32_t destination, void *buffer, size_t len) {
  ...
  switch(icmp_msg->header.type){
    ...
    case ICMP_TYPE_ECHO_REQUEST:{
      ...
      reply_msg->echo.identify = icmp_msg->echo.identify; // copy the identify number
      reply_msg->echo.sequence = icmp_msg->echo.sequence; // copy the sequence number
      memcpy(&reply_msg->echo.data, &icmp_msg->echo.data, len - (sizeof(icmp_header) + sizeof(icmp_echo))); // copy the data
      reply_msg->header.checksum = checksum_16(reinterpret_cast<uint16_t *>(reply_mybuf->buffer), reply_mybuf->len); // calcuration of checksum
...
void send_icmp_time_exceeded(uint32_t dest_addr, uint32_t src_addr, uint8_t code, void *error_ip_buffer, size_t len){
  ...
  // Allocate ICMP header + area of message + error packet section (IP header + bytes)
