...
int main(){
  ...
  for(ifaddrs *tmp = addrs; tmp; tmp = tmp->ifa_next){
    if(tmp->ifa_addr && tmp->ifa_addr->sa_family == AF_PACKET){
      ...
      auto *dev = (net_device *) calloc(1, sizeof(net_device) + sizeof(net_device_data)); // Allocate an area for net_device and an area for net_device_data
      ...
      memcpy(dev->mac_addr, &ifr.ifr_hwaddr.sa_data[0], 6); // Set MAC address in net_device
