...
enum class nat_direction { outgoing, incoming };

enum class nat_protocol { udp, tcp, icmp };
...
// Set of NAT tables for ICMP, UDP, TCP
struct nat_entries {
...
// NAT devices with ip_device inside NAT
struct nat_device {
  uint32_t outside_addr;  // IP address of conversion destination
  nat_entries *entries;   // NAT table
};
